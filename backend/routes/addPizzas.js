...
router.post('/addPizza', async (req, res) => {
    ...
    try {
      ...
      const insertPizzaQuery = 'INSERT INTO pizzas (name, price, image) VALUES (?, ?, ?)';
      const [result] = await connection.query(insertPizzaQuery, [name, price, image]);
      ...
      const insertToppingPromises = toppings.map(async (topping) => {
        ...
        await connection.query(insertToppingQuery, [pizzaId, toppingResult[0].id]);
      });
