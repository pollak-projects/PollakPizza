...
const fetchUserOrders = async (userId) => {
  try {
    const response = await axios.get(
      `http://localhost:3061/admin/orders/${userId}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
...
const addPizza = async () => {
  try {
    const selectedToppingsNames = selectedToppings.value
      .map((id) => {
...
const updatePizza = async (pizza) => {
  try {
    ...
    await axios.put(
      `http://localhost:3061/admin/updatePizza/${pizza.id}`,
      {
        name: pizza.name,
        price: pizza.price,
        image: pizza.image,
  ...
  <body>
    <div class="admin-page">
              ...
              <td>
                <div v-for="topping in toppings" :key="topping.id">
                  <input
                    ...
                    :value="topping.id"
                    :checked="
                      pizza.toppings && pizza.toppings.includes(topping.name)
                    "
                    @change="togglePizzaTopping(pizza, topping.id)"
                  />
                  <label :for="`topping-${topping.id}-${pizza.id}`">{{
                    topping.name
                  }}</label>
                </div>
              </td>
      ...
      <router-link to="/orders" class="orders-link"
        >Rendelések megtekintése</router-link
      >
